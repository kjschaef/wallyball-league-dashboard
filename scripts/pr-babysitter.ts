...
async function auditPRs() {
  ...
  if (!pulls || !Array.isArray(pulls)) {
    console.log('No open PRs found or error fetching PRs.');
    return;
  }
  ...
  for (const pr of pulls) {
    ...
    // Fetch branch protection to identify required checks
    const branchProtection = await githubFetch(`/repos/${REPO}/branches/${pr.base.ref}/protection`);
    const requiredCheckContexts: string[] = branchProtection?.required_status_checks?.contexts || [];
    ...
    for (const run of allCheckRuns) {
      const isRequired = requiredCheckContexts.length === 0 || requiredCheckContexts.includes(run.name);
      if (run.status !== 'completed') {
        ...
        if (isRequired) failures.push(`Check Run: ${run.name} (${run.conclusion})`);
      }
    }
    ...
    for (const status of latestStatuses.values()) {
      const isRequired = requiredCheckContexts.length === 0 || requiredCheckContexts.includes(status.context);
      if (status.state === 'pending') {
        if (isRequired) pending = true;
      } else if (status.state === 'failure' || status.state === 'error') {
        if (isRequired) failures.push(`Status: ${status.context}`);
      }
    }
    ...
    if (failures.length > 0) {
      ...
      const alreadyCommented = comments && Array.isArray(comments) && comments.some((c: any) =>
