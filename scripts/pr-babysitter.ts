...
async function auditPRs() {
  ...
  for (const pr of pulls) {
    ...
    const latestStatuses = new Map<string, { state: string; context: string }>();
    for (const status of combinedStatuses as { state: string; context: string }[]) {
    ...
    const labels = (pr.labels as { name: string }[]).map((l) => l.name);
    ...
    if (failures.length > 0) {
      ...
      const alreadyCommented = (comments as { body: string; user: { login: string } }[] | null)?.some((c) =>
    ...
    for (const review of reviews as { state: string; user: { login: string } }[]) {
      // Only keep states that affect the PR's mergeability
...
// Detect if script is run directly
const isMain = process.argv[1] && (process.argv[1].endsWith('pr-babysitter.ts'));
